    ...
    try {
        ...
        if (lockCheck.rowCount > 0) {
            ...
            return cb(null, {
                ...
                error: "This page is currently locked by another user"
            });
        }
...
responder.on('create-home', async (req, cb) => {
    ...
    try {
        ...
        // -----------------------------
        if (!['home', 'HOME'].includes(page_key)) {
            return cb(null, {
                header_type: "ERROR",
                message_visibility: true,
                status: false,
                code: 2001,
                message: "Validation failed",
                error: "Only home page is allowed"
            });
        }
        ...
        const existingPage = await client.query(
            `SELECT page_id, is_active, is_deleted FROM pages WHERE LOWER(page_key) = 'home'  ORDER BY created_at DESC LIMIT 1`
        );
...
responder.on('create-aboutus', async (req, cb) => {
    ...
    try {
        ...
        // -----------------------------
        if (!['about us', 'ABOUT US'].includes(page_key)) {
            return cb(null, {
                header_type: "ERROR",
                message_visibility: true,
                status: false,
                code: 2001,
                message: "Validation failed",
                error: "Only About Us page is allowed"
            });
        }
        ...
        // -----------------------------
        const existingPage = await client.query(
            `SELECT page_id, is_active, is_deleted FROM pages WHERE LOWER(page_key) = 'about us'  ORDER BY created_at DESC LIMIT 1`
        );
...
responder.on('update-aboutus', async (req, cb) => {
    ...
    try {
        ...
        for (const section of sections) {
            ...
            if (secRes.rowCount === 0 || !secRes.rows[0].is_active || secRes.rows[0].is_deleted) {
...
responder.on('listbypagekey-aboutus', async (req, cb) => {
    try {
        const { page_key } = req;
...
responder.on('create-contactus', async (req, cb) => {
    ...
    try {
        ...
        // -----------------------------
        if (!['contact us', 'CONTACT US'].includes(page_key)) {
            return cb(null, {
                header_type: "ERROR",
                message_visibility: true,
                status: false,
                code: 2001,
                message: "Validation failed",
                error: "Only Contact Us page is allowed"
            });
        }
        ...
        const activeContactUsCheck = await client.query(
            `SELECT 1 
             FROM pages 
             WHERE LOWER(page_key) = 'contact us' 
               AND is_active = TRUE AND is_deleted = FALSE`
        );
        ...
        // -----------------------------
        const existingPage = await client.query(
            `SELECT page_id, is_active, is_deleted FROM pages WHERE LOWER(page_key) = 'contact us'  ORDER BY created_at DESC LIMIT 1`
        );
...
responder.on('listbypagekey-contactus', async (req, cb) => {
    try {
        const { page_key } = req;
...
responder.on('create-buyerhome', async (req, cb) => {
    ...
    try {
        ...
        //  PAGE VALIDATION
        // -----------------------------
        if (!['buyer home', 'BUYER HOME'].includes(page_key)) {
            return cb(null, {
                header_type: "ERROR",
                message_visibility: true,
                status: false,
                code: 2001,
                message: "Validation failed",
                error: "Only Buyer Home page is allowed"
            });
        }
        ...
        const activeBuyerHomeCheck = await client.query(
            `SELECT 1 
             FROM pages 
             WHERE LOWER(page_key) = 'buyer home' 
               AND is_active = TRUE AND is_deleted = FALSE`
        );
        ...
        // -----------------------------
        const existingPage = await client.query(
            `SELECT page_id, is_active, is_deleted FROM pages WHERE LOWER(page_key) = 'buyer home'  ORDER BY created_at DESC LIMIT 1`
        );
...
responder.on('create-sellerhome', async (req, cb) => {
    ...
    try {
        ...
        //  PAGE VALIDATION
        // -----------------------------
        if (!['seller home', 'SELLER HOME'].includes(page_key)) {
            return cb(null, {
                header_type: "ERROR",
                message_visibility: true,
                status: false,
                code: 2001,
                message: "Validation failed",
                error: "Only Seller Home page is allowed"
            });
        }
        ...
        const activeSellerHomeCheck = await client.query(
            `SELECT 1 
             FROM pages 
             WHERE LOWER(page_key) = 'seller home' 
               AND is_active = TRUE AND is_deleted = FALSE`
        );
        ...
        // -----------------------------
        const existingPage = await client.query(
            `SELECT page_id, is_active, is_deleted FROM pages WHERE LOWER(page_key) = 'seller home'  ORDER BY created_at DESC LIMIT 1`
        );
